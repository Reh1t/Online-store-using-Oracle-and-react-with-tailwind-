...
const EditVendor = () => {
  ...
  const tempName = localStorage.getItem('Vname');
  const tempEmail = localStorage.getItem('Vemail');
  const tempCnic = localStorage.getItem('Vcnic');
  const tempContact = localStorage.getItem('Vcontact');
  const tempAddress = localStorage.getItem('Vaddress');
  ...
  function check()
  {
    fetch('http://localhost:5000/query', {
      ...
      body: JSON.stringify({
        query: "Select Vendorid, name, email, cnic, contact, address, password from vendor where vendorid ="+vendorId
      })
    })
    .then(res => res.text()) // Get the response as a string
    .then(data => {
        const dataArray = data.split(',');
       if(dataArray[0] == vendorId && dataArray[1] == name && dataArray[2] == email && dataArray[3] == cnic && dataArray[4] == contact && dataArray[5] == address && dataArray[6] == password)
       {
        localStorage.setItem('vendorid', dataArray[0]);
        localStorage.setItem('Vname', dataArray[1]);
        localStorage.setItem('Vemail', dataArray[2]);
        localStorage.setItem('Vcnic', dataArray[3]);
        localStorage.setItem('Vcontact', dataArray[4]);
        localStorage.setItem('Vaddress', dataArray[5]);
  ...
  const handleSubmit = (e) => {
    ...
    fetch('http://localhost:5000/query', {
      ...
      body: JSON.stringify({
        query: "DECLARE ecount NUMBER := 0; ccount NUMBER := 0; BEGIN Select count(*) INTO ecount from vendor where email='"+email+"' AND VendorID <> "+vendorId+"; Select count(*) INTO ccount from vendor where cnic='"+cnic+"' AND vendorID <> "+vendorId+"; if(ecount = 0 AND ccount = 0) then Update vendor set name ='"+name+"', cnic='"+cnic+"', contact='"+contact+"', address='"+address+"', email='"+email+"', password='"+password+"' where vendorID = "+vendorId+"; end if; END;"
      })
    })
    .then(res => res.text()) // Get the response as a string
    .then(data => {
