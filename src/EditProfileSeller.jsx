...
const EditSeller = () => {
  ...
  const tempCnic = localStorage.getItem('cnic');
  const tempContact = localStorage.getItem('contact');
  ...
  function check()
  {
    fetch('http://localhost:5000/query', {
      ...
      body: JSON.stringify({
        query: "Select sellerid, name, email, cnic, contact, address, password from seller where sellerid ="+sellerId
      })
    })
    .then(res => res.text()) // Get the response as a string
    .then(data => {
        const dataArray = data.split(',');
       if(dataArray[0] == sellerId && dataArray[1] == name && dataArray[2] == email && dataArray[3] == cnic && dataArray[4] == contact && dataArray[5] == address && dataArray[6] == password)
       {
        localStorage.setItem('sellerid', dataArray[0]);
        localStorage.setItem('name', dataArray[1]);
        localStorage.setItem('email', dataArray[2]);
        localStorage.setItem('cnic', dataArray[3]);
        localStorage.setItem('contact', dataArray[4]);
        localStorage.setItem('address', dataArray[5]);
  ...
  const handleSubmit = (e) => {
    ...
    fetch('http://localhost:5000/query', {
      ...
      body: JSON.stringify({
        query: "DECLARE ecount NUMBER := 0; ccount NUMBER := 0; BEGIN Select count(*) INTO ecount from seller where email='"+email+"' AND sellerid <> "+sellerId+"; Select count(*) INTO ccount from seller where cnic='"+cnic+"' AND sellerid <> "+sellerId+"; if(ecount = 0 AND ccount = 0) then Update seller set name ='"+name+"', cnic='"+cnic+"', contact='"+contact+"', address='"+address+"', email='"+email+"', password='"+password+"' where sellerid = "+sellerId+"; end if; END;"
      })
    })
    .then(res => res.text()) // Get the response as a string
    .then(data => {
      ...
      if(data.includes(email)) {
        ...
        const dataArray = data.split(',');
        localStorage.setItem('seller', dataArray[0]);
        localStorage.setItem('name', dataArray[1]);
